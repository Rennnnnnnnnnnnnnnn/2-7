...
export const getTransactions = async (req, res, next) => {
    ...
    try {
        const [results] = await db.execute(
            'SELECT * FROM transactions WHERE batch_id = ? ORDER BY transaction_date DESC',
            [batchId]  // Use the batchId as a parameter to filter transactions
        );
...
export const getTransactionById = async (req, res, next) => {
    try {
        ...
        res.json(result[0]);  // Send the single transaction data
    } catch (err) {
...
export const updateTransaction = async (req, res, next) => {
    try {
        const { id } = req.params;  // Get ID from URL parameters
        const { transactionDate, transactionType, itemType, contactName, quantity, pricePerUnit, totalCost, } = req.body;

        // Validate if required fields are provided
        if (!transactionDate || !transactionType || !itemType || !quantity || !pricePerUnit || !totalCost) {
            const error = new Error('All fields are required.');
            error.status = 400;
            return next(error);
        }
        ...
        const query = `UPDATE transactions 
                       SET transaction_date = ?, transaction_Type = ?, item_type = ?, contact_name = ?, quantity = ?, price_per_unit = ?, total_cost = ?
                       WHERE transaction_id = ?`;

        const [result] = await db.execute(query, [transactionDate, transactionType, itemType, contactName, quantity, pricePerUnit, totalCost, id]);
